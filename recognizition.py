...
from utils.cap_cam import Camera
...
class YOLO(formType, baseType):
    def __init__(self):
        ...
        # SpinBox
        # spin change then slider change too
        self.con_num.valueChanged.connect(self.SliderChange)
        self.iou_num.valueChanged.connect(self.SliderChange)
        self.numcon_ = self.con_num.value()
        self.numiou_ = self.iou_num.value()
        ...
        self.yolo_thread.weights = "./pt/%s" % self.model_type
    ...
    def SliderChange(self):
        self.numcon_ = self.con_num.value()
        self.numiou_ = self.iou_num.value()
        self.con_slider.setValue(self.numcon_ * 100)
        self.yolo_thread.conf = self.numcon_
        self.iou_slider.setValue(self.numiou_ * 100)
        self.yolo_thread.iou = self.numiou_
    ...
    def Selectfile(self):
        file, _ = QFileDialog.getOpenFileName(
            ...
            "选择你要上传的图片/视频",  # 标题
            "./media",  # 默认打开路径为当前路径
            "图片/视频类型 (*.jpg *.jpeg *.png *.bmp *.dib *.jpe *.jp2 *.mp4 *.avi)"  # 选择类型过滤项，过滤内容在括号中
        )
        if file == "":
            pass
        else:
            self.inputPath = file  # 源文件路径
    ...
    def Import(self):
        file, _ = QFileDialog.getOpenFileName(
            ...
            "选择你要导入的模型",  # 标题
            "./",  # 默认打开路径为当前路径
            "模型类型 (*.pt)"  # 选择类型过滤项，过滤内容在括号中
        )
        if file == "":
            pass
        else:
            shutil.copy(file, self.pt_Path)
            QMessageBox.information(self, '提示', '模型导入成功，可在Models中选择！')
    ...
    def Export(self):
        self.OutputDir, _ = QFileDialog.getSaveFileName(
            ...
            "导出图片/视频",  # 标题
            r"./result",  # 起始目录
            "文件类型 (*.jpg *.jpeg *.png *.bmp *.dib *.jpe *.jp2 *.mp4 *.avi)"  # 选择类型过滤项，过滤内容在括号中
        )
        if self.output == "":
    ...
    def chose_cam(self):
        try:
            self.stop()
            # MessageBox(self.CloseButton, title='Tips', text='Loading camera', time=2000, auto=True).exec_()
            ...
            x = self.groupBox.mapToGlobal(self.camera.pos()).x()
            y = self.groupBox.mapToGlobal(self.camera.pos()).y()
            y = y + self.camera.frameGeometry().height()
            pos = QPoint(x, y)
            action = popMenu.exec_(pos)
            if action:
                self.inputPath = action.text()
                glo.set_value('inputPath', self.inputPath)
                self.foot_print('Loading camera：{}'.format(action.text()))
        except Exception as e:
            self.foot_print('%s' % e)
